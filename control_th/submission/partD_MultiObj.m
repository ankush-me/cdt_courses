...
disp_title('Multi-Objective LQR')
...
disp_yellow(['Eigenvalues of [A - BK] = ', num2str(eig(Acl)')])
...
disp_yellow(['Eigenvalues of [A(-) - BK] = ', num2str(eig(Am_cl)')])
if all(real(eig(Am_cl)) < 0)
	disp_red('   >> 90% of 7.74 closed-loop system is stable')
else   
	disp_red('   >> 90% of 7.74 closed-loop system is NOT stable')
end
disp(' ')

disp_yellow(['Eigenvalues of [A(+) - BK] = ', num2str(eig(Ap_cl)')])
if all(real(eig(Ap_cl)) < 0)
	disp_red('   >> 110% of 7.74 closed-loop system is stable')
else
	disp_red('   >> 110% of 7.74 closed-loop system is NOT stable')
end
