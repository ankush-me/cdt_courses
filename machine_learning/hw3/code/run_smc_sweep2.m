function [z] = run_smc_sweep2(num_particles, data, K, alpha, beta, Lambda_0, nu)
...
for n = 1:N
    ...
    for ip = 1:num_particles
    	...
    	for ik=1:K
	    	log_proposal(ik) = log_predictive_collapsed_gmm(x_n', K, N, Ns{ip}(ik), mu{ip}(:,ik), S{ip,ik}, alpha, beta, Lambda_0, nu);
	    end
